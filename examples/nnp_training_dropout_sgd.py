...
drop_prop = 0.1
tensorboard_folder = 'runs/dropout01_noLastLayer_sgd'
...
SGD = torch.optim.SGD([
    ...
    {'params': [H_network[3].bias]},
    {'params': [H_network[6].bias]},
    {'params': [H_network[8].bias]},
    # C networks
    {'params': [C_network[0].bias]},
    {'params': [C_network[3].bias]},
    {'params': [C_network[6].bias]},
    {'params': [C_network[8].bias]},
    # N networks
    {'params': [N_network[0].bias]},
    {'params': [N_network[3].bias]},
    {'params': [N_network[6].bias]},
    {'params': [N_network[8].bias]},
    # O networks
    {'params': [O_network[0].bias]},
    {'params': [O_network[3].bias]},
    ...
    {'params': [H_network[3].weight]},
    {'params': [H_network[6].weight]},
    {'params': [H_network[8].weight]},
    # C networks
    {'params': [C_network[0].weight]},
    {'params': [C_network[3].weight]},
    {'params': [C_network[6].weight]},
    {'params': [C_network[8].weight]},
    # N networks
    {'params': [N_network[0].weight]},
    {'params': [N_network[3].weight]},
    {'params': [N_network[6].weight]},
    {'params': [N_network[8].weight]},
    # O networks
    {'params': [O_network[0].weight]},
    {'params': [O_network[3].weight]},
...
print("training starting from epoch", SGD_scheduler.last_epoch + 1)
max_epochs = 500
early_stopping_learning_rate = 1.0E-5
best_model_checkpoint = os.path.join(tensorboard_folder, 'best.pt')

for _ in range(SGD_scheduler.last_epoch + 1, max_epochs):
    rmse = validate()
    print('RMSE:', rmse, 'at epoch', SGD_scheduler.last_epoch + 1)
    ...
    # checkpoint
    if SGD_scheduler.is_better(rmse, SGD_scheduler.best):
        torch.save(nn.state_dict(), best_model_checkpoint)

    # AdamW_scheduler.step(rmse)
    SGD_scheduler.step(rmse)

    tensorboard.add_scalar('validation_rmse', rmse, SGD_scheduler.last_epoch)
    tensorboard.add_scalar('best_validation_rmse', SGD_scheduler.best, SGD_scheduler.last_epoch)
    # tensorboard.add_scalar('learning_rate', learning_rate, AdamW_scheduler.last_epoch)

    for i, (batch_x, batch_y) in tqdm.tqdm(
            enumerate(training),
            total=len(training),
            desc="epoch {}".format(SGD_scheduler.last_epoch)
    ):
        ...
        tensorboard.add_scalar('batch_loss', loss, SGD_scheduler.last_epoch * len(training) + i)
